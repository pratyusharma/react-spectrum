...
By default, buttons use an indeterminate spinner for the loading state.
...
For a nostalgic terminal-style loading experience, you can use the `loadingStyle="terminal"` prop. This replaces the button text with an animated cycling dots pattern (e.g., "Loading", "Loading.", "Loading..", "Loading...", "Loading....").
...
- **`loadingStyle`**: Set to `"terminal"` to enable the terminal animation (default: `"spinner"`)
- **`loadingText`**: Custom text to display during loading (default: `"Loading"`)
- **`loadingSpeed`**: Animation speed in milliseconds per frame (default: `500`)
- **`loadingDots`**: Maximum number of dots in the animation cycle (default: `4`)

### Accessibility

The terminal loading animation maintains full accessibility support:

- Screen readers announce the loading state via `aria-live="polite"`
- The button remains focusable during loading
- Proper ARIA labels describe the loading state
...
<Button 
  ...
  aria-label="Save document - currently saving changes"
>
  Save
</Button>
```
