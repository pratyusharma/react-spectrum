...
Buttons support two loading animation styles that can be configured with the `loadingStyle` prop:

- **Custom** (default): You provide your own loading indicator (typically a [ProgressBar](ProgressBar.html) component)
- **Terminal**: A built-in terminal-style text animation that cycles through dots

#### Custom Loading Indicator

A [ProgressBar](ProgressBar.html) component is required to show the pending state correctly when using the default loading style.
...
The terminal loading style provides a built-in nostalgic animation that cycles through text and dots, reminiscent of classic terminal interfaces.

```tsx example
import {useState} from 'react';

function TerminalPendingButton() {
  let [isPending, setPending] = useState(false);
...
The terminal loading style supports the following customization props:

- `terminalText`: The base text to animate (default: "Loading")
- `terminalMaxDots`: Maximum number of dots to append (default: 4)
- `terminalSpeed`: Animation speed in milliseconds (default: 500)
